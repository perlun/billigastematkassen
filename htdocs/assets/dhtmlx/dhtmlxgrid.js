...
function dhtmlXGridObject(a)
{
    ...
    if (this.hdr.rows[0])
    {
      ...
      else
      {
        var d = !this._wthB && (this.entBox.cmp || this._delta_x) && (this.skin_name || "").indexOf("dhx") == 0 && !e ? 2 : 0,
          ...
          p = this.setColumnSizes(n -
            (l ? k : 0) - (this._correction_x || 0)),
    ...
    if (a.tagName == "TD")
    {
      ...
      if (d == 1)
      {
        ...
        for (var p = 0; p < this.rowsCol.length; p++) if (p >=
            n && p <= o) if (this.rowsCol[p] && !this.rowsCol[p]._sRow) this.rowsCol[p].className.indexOf("rowselected") == -1 && this.callEvent("onBeforeSelect", [this.rowsCol[p].idd, g]) && (this.rowsCol[p].className += " rowselected", this.selectedRows[this.selectedRows.length] = this.rowsCol[p]);
  ...
  this.scrollPage = function (a)
  {
    if (this.rowsBuffer.length)
    {
      var b = this._realfake ? this._fake : this,
        c = Math.floor((b._r_select || this.getRowIndex(this.row.idd) ||
  ...
  this.copyRowContent = function (a, b)
  {
    ...
    else for (var d = 0; d < c.cells.length; d++) this.cells(b, d).setValue(this.cells(a, d).getValue()); if (!_isIE) this.getRowById(a).cells[0].height = c.cells[0].offsetHeight
  };
...
dhtmlXGridObject.prototype = {
  ...
  _createHRow: function (a, b)
  {
    ...
    else
    {
      ...
      if (typeof a[0][j] == "object") m.appendChild(a[0][j]);
      else if (m.innerHTML = this.forceDivInHeader ? "<div class='hdrcell'>" + (a[0][j] || "&nbsp;") + "</div>" : a[0][j] || "&nbsp;", (a[0][j] || "").indexOf("#") != -1) if (d = a[0][j].match(/(^|{)#([^}]+)(}|$)/))
        {
          var n = "_in_header_" +
            d[2];
  ...
  _refreshFromXML: function (a)
  {
      ...
      a.doXPath("//row"), i = 0; i < e.length; i++)
    {
      ...
      if (this.rowsAr[f] && this.rowsAr[f].tagName != "TR") this._h2 ? this._h2.get[f].buff.data = j : this.rowsBuffer[this.getRowIndex(f)].data = j, this.rowsAr[f] = j;
      else if (this.rowsAr[f]) this._process_xml_row(this.rowsAr[f], j, -1), this._postRowProcessing(this.rowsAr[f], !0);
  ...
  _postRowProcessing: function (a, b)
  {
    ...
    for (var c = 0, d = 0; d < a.childNodes.length; d++)
    {
      var e =
        a.childNodes[d],
        i = e._cellIndex,
        j = e._attrs.style || a._attrs.style;
      j && (e.style.cssText += ";" + j);
  ...
  _process_xml: function (a)
  {
    if (!a.doXPath)
    {
      ...
      typeof a == "string" ? b.loadXMLString(a) : (b.xmlDoc = a.responseXML ? a :
      {}, b.xmlDoc.responseXML =
        a);
      a = b
    }
    ...
    if (c.tagName == this.xml.top)
    {
      ...
      var e = a.doXPath(this.xml.row, c),
        i = parseInt(a.doXPath("//" + this.xml.top)[0].getAttribute("pos") || 0),
        j = parseInt(a.doXPath("//" + this.xml.top)[0].getAttribute("total_count") || 0),
        f = !1;
  ...
  _process_json: function (a, b)
  {
    ...
    if (b == "js")
    {
      ...
      for (var c = 0; c < a.length; c++)
      {
        ...
        }), this.rowsAr[e] = a[c]; if (a.dhx_security) dhtmlx.security_key = a.dhx_security;
    this.render_dataset();
    this._parsing = !1
  },
  render_dataset: function (a, b)
  {
    ...
    if (this._srnd && !this._fillers)
    {
      ...
      {
        var j = _isIE ? Math.min(i, 5E4) : i,
          f = this._add_filler(b, j);
...
(function ()
{
  ...
  function e(a, b)
  {
    ...
    for (var d = 1, e = [], k = 0; k < b.length; k++) if (typeof b[k] == "object" && b[k].length) for (var i = 0; i < b[k].length; i++) c(k, i, b[k][i]);
      else c(k,
          0, b[k]);
